import { generateSalesOrderHTML } from './sales-order-html-template';
...
export async function generateSalesOrderPDF(
  ...
  const soResult = await query(
    `SELECT so.*,
            ...
            c.contact_person as customer_contact_person, c.email as customer_email, c.phone as customer_phone,
     ...
     JOIN company_profiles comp ON so.company_id = comp.company_id
     WHERE so.so_id = $1 AND so.user_id = $2`,
    [soId, userId]
  );
  ...
  const salesOrderData = normalizeObject(soResult.rows[0]);
  const itemsData = normalizeObject(itemsResult.rows);

  // 4️⃣ Generate HTML
  const html = generateSalesOrderHTML({
