...
<body>
  <div class="invoice-page">
    ...
    <!-- Credit Note Specific: Ref Box -->
    <div class="ref-box">
        <strong>Original Invoice:</strong> ${safeText(creditNote.original_invoice_number)} dated ${formatDate(creditNote.original_invoice_date)}
    </div>
    
    <!-- Credit Note Specific: Reason Box -->
    <div class="reason-box">
        <strong>Reason:</strong> ${safeText(creditNote.reason)}
    </div>
    ...
    <div class="main-box">
      ...
      ? `<img src="${safeText(creditNote.company_logo)}" alt="Company Logo" />`
      : `<div class="logo-placeholder"><div class="logo-triangle"></div></div>`
            ...
            <div class="company-info">
              ...
              ${creditNote.company_address_line1 ? `<div>${safeText(creditNote.company_address_line1)},</div>` : ''}
              ${creditNote.company_address_line2 ? `<div>${safeText(creditNote.company_address_line2)}</div>` : ''}
              ${creditNote.company_city ? `<div>${safeText(creditNote.company_city)}</div>` : ''}
              <div>GSTIN/UIN: ${safeText(creditNote.company_gst) || 'N/A'}</div>
              <div>State Name : ${safeText(creditNote.company_state)}${creditNote.company_state_code ? `, Code : ${safeText(creditNote.company_state_code)}` : ''}</div>
              ${creditNote.company_email ? `<div>E-Mail : ${safeText(creditNote.company_email)}</div>` : ''}
            </div>
          </div>
          ...
          <div class="buyer-section">
            ...
            <div class="buyer-name">${safeText(creditNote.customer_name)}</div>
            <div class="buyer-details">
              ...
              ${creditNote.customer_city ? `${safeText(creditNote.customer_city)}<br>` : ''}
              ${creditNote.customer_gst ? `GSTIN/UIN : ${safeText(creditNote.customer_gst)}<br>` : ''}
              State Name : ${safeText(creditNote.customer_state)}${creditNote.customer_state_code ? `, Code : ${safeText(creditNote.customer_state_code)}` : ''}
            </div>
          </div>
        </div>
        ...
        <div class="right-col">
          <div class="detail-row">
            <div class="detail-cell w25">Credit Note No.</div>
            <div class="detail-cell w25">${safeText(creditNote.credit_note_number)}</div>
            <div class="detail-cell w25">Dated</div>
            <div class="detail-cell w25">${formatDate(creditNote.credit_note_date)}</div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Original Inv. No.</div>
            <div class="detail-cell w25">${safeText(creditNote.original_invoice_number)}</div>
            <div class="detail-cell w25">Original Inv. Date</div>
            <div class="detail-cell w25">${formatDate(creditNote.original_invoice_date)}</div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Place of Supply</div>
            <div class="detail-cell w25">${safeText(creditNote.place_of_supply)}</div>
    ...
    <div class="items-section">
      ...
      return `
        ...
        ${isIntraState && Number(creditNote.cgst_total) > 0 ? `
        <div class="item-row gst-row">
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(creditNote.cgst_total))}</div>
        </div>
        <div class="item-row gst-row">
          <div class="col-sl"></div>
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(creditNote.sgst_total))}</div>
        </div>
        ` : ''}

        ${!isIntraState && Number(creditNote.igst_total) > 0 ? `
        <div class="item-row gst-row">
          <div class="col-sl"></div>
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(creditNote.igst_total))}</div>
        </div>
        ` : ''}
      </div>
      ...
      <div class="total-row">
        ...
        <div class="col-rate"></div>
        <div class="col-per"></div>
        <div class="col-amt grand-total">₹ ${formatINRForPDF(Number(creditNote.grand_total))}</div>
      </div>
    </div>
    ...
    <table class="tax-table">
      ...
      <tbody>
        ...
        <tr style="font-weight:bold;">
          <td>Total</td>
          <td class="text-right">${formatINRForPDF(Number(creditNote.subtotal))}</td>
          ${isIntraState ? `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(creditNote.cgst_total))}</td>
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(creditNote.sgst_total))}</td>
          ` : `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(creditNote.igst_total))}</td>
          `}
          <td class="text-right">${formatINRForPDF(Number(creditNote.tax_total))}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="footer">
      ...
      <div class="footer-right">
        <div>for <strong>${safeText(creditNote.company_name)}</strong></div>
        <div class="signature">Authorised Signatory</div>
      </div>
    </div>

    <!-- Computer Generated Note -->
    <div class="computer-note">This is a Computer Generated Credit Note</div>
  </div>
</body>
</html>
  `.trim();
