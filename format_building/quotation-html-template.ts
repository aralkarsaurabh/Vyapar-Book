...
export function generateQuotationHTML({ quotation, items }: QuotationData): string {
  const isIntraState = quotation.company_state === quotation.customer_state;
...
<head>
  ...
  <title>Quotation - ${safeText(quotation.quotation_number)}</title>
  <style>
    @page {
      size: A4;
      margin: 10mm;
...
<body>
  <div class="invoice-page">
    ...
    <div class="main-box">
      ...
      ? `<img src="${safeText(quotation.company_logo)}" alt="Company Logo" />`
      : `<div class="logo-placeholder"><div class="logo-triangle"></div></div>`
    }
            </div>
            <div class="company-info">
              <div class="company-name">${safeText(quotation.company_name)}</div>
              ${quotation.company_address_line1 ? `<div>${safeText(quotation.company_address_line1)},</div>` : ''}
              ${quotation.company_address_line2 ? `<div>${safeText(quotation.company_address_line2)}</div>` : ''}
              ${quotation.company_city ? `<div>${safeText(quotation.company_city)}</div>` : ''}
              <div>GSTIN/UIN: ${safeText(quotation.company_gst) || 'N/A'}</div>
              <div>State Name : ${safeText(quotation.company_state)}${quotation.company_state_code ? `, Code : ${safeText(quotation.company_state_code)}` : ''}</div>
              ${quotation.company_email ? `<div>E-Mail : ${safeText(quotation.company_email)}</div>` : ''}
            </div>
          </div>

          <div class="consignee-section">
            <div class="buyer-label">Consignee (Ship to)</div>
            <div class="buyer-name">${safeText(quotation.customer_name)}</div>
            <div class="buyer-details">
              ${quotation.customer_address_line1 ? `${safeText(quotation.customer_address_line1)}<br>` : ''}
              ${quotation.customer_address_line2 ? `${safeText(quotation.customer_address_line2)}<br>` : ''}
              ${quotation.customer_city ? `${safeText(quotation.customer_city)}<br>` : ''}
              ${quotation.customer_gst ? `GSTIN/UIN : ${safeText(quotation.customer_gst)}<br>` : ''}
              State Name : ${safeText(quotation.customer_state)}, Code : ${safeText(quotation.customer_state_code) || ''}
            </div>
          </div>
        ...
        <div class="right-col">
          <div class="detail-row">
            <div class="detail-cell w25">Quotation No.</div>
            <div class="detail-cell w25">${safeText(quotation.quotation_number)}</div>
            <div class="detail-cell w25">Dated</div>
            <div class="detail-cell w25">${formatDate(quotation.quotation_date)}</div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Valid Until</div>
            <div class="detail-cell w25">${quotation.valid_until ? formatDate(quotation.valid_until) : ''}</div>
          ...
          <div class="detail-row">
            <div class="detail-cell w25">Customer PO No.</div>
            <div class="detail-cell w25"></div>
            <div class="detail-cell w25">Dated</div>
            <div class="detail-cell w25"></div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Despatch Doc No.</div>
            <div class="detail-cell w25"></div>
          ...
          <div class="detail-row">
            ...
            <div class="detail-cell w25">${safeText(quotation.place_of_supply) || ''}</div>
          </div>
    ...
    <div class="items-section">
      ...
      return `
        ...
        ${quotation.quotation_type === 'GST' && isIntraState ? `
        <div class="item-row gst-row">
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(quotation.cgst_total))}</div>
        </div>
        <div class="item-row gst-row">
          <div class="col-sl"></div>
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(quotation.sgst_total))}</div>
        </div>
        ` : ''}
        ...
        <div class="item-row gst-row">
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(quotation.igst_total))}</div>
        </div>
        ` : ''}
      </div>
      ...
      <div class="total-row">
        ...
        <div class="col-rate"></div>
        <div class="col-per"></div>
        <div class="col-amt grand-total">₹ ${formatINRForPDF(Number(quotation.grand_total))}</div>
      </div>
    </div>
    ...
    <div class="amount-words">
      <div class="amount-words-left">
        ...
        <span class="amount-words-value">${amountToWords(Number(quotation.grand_total))}</span>
      </div>
      <div class="amount-words-right">E. & O.E</div>
    </div>

    <!-- Tax Summary Table -->
    ${quotation.quotation_type === 'GST' ? `
    <table class="tax-table">
      ...
      <tbody>
        ...
        <tr style="font-weight:bold;">
          <td>Total</td>
          <td class="text-right">${formatINRForPDF(Number(quotation.subtotal))}</td>
          ${isIntraState ? `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(quotation.cgst_total))}</td>
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(quotation.sgst_total))}</td>
          ` : `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(quotation.igst_total))}</td>
          `}
          <td class="text-right">${formatINRForPDF(Number(quotation.tax_total))}</td>
        </tr>
      </tbody>
    </table>

    <div class="tax-words">
      <strong>Tax Amount (in words) :</strong> ${amountToWords(Number(quotation.tax_total))}
    </div>
    ` : ''}
    ...
    <div class="footer">
      <div class="footer-left">
        ...
        This quotation is valid until ${quotation.valid_until ? formatDate(quotation.valid_until) : 'the date mentioned above'}. All prices are subject to change.
        ${quotation.notes ? `<br><br><strong>Notes:</strong><br>${safeText(quotation.notes)}` : ''}
        ${quotation.terms_and_conditions ? `<br><br><strong>Terms & Conditions:</strong><br>${safeText(quotation.terms_and_conditions)}` : ''}
        ${quotation.bank_name || quotation.bank_account_number ? `
        <br><br><strong>Bank Details:</strong><br>
        ${quotation.bank_name ? `Bank Name: ${safeText(quotation.bank_name)}<br>` : ''}
        ${quotation.bank_account_number ? `A/c No: ${safeText(quotation.bank_account_number)}<br>` : ''}
        ${quotation.bank_ifsc ? `IFSC Code: ${safeText(quotation.bank_ifsc)}<br>` : ''}
        ${quotation.bank_branch ? `Branch: ${safeText(quotation.bank_branch)}` : ''}
        ` : ''}
      </div>
      <div class="footer-right">
        <div>for <strong>${safeText(quotation.company_name)}</strong></div>
        <div class="signature">Authorised Signatory</div>
      </div>
    </div>

    <div class="computer-note">This is a Computer Generated Quotation</div>
  </div>
</body>
</html>
  `.trim();
