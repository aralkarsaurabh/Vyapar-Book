...
function generateInvoiceHTML({ invoice, items }) {
  ...
  // Generate GST rows
  let gstRows = '';
  if (invoice.invoice_type === 'GST' && isIntraState) {
    gstRows = `
  ...
  } else if (invoice.invoice_type === 'GST' && !isIntraState) {
    gstRows = `
        <div class="item-row gst-row">
          <div class="col-sl"></div>
  ...
  <div class="invoice-page">
    ...
    <div class="main-box">
      <div class="main-row">
        <div class="left-col">
          <div class="company-header">
            ...
            <div class="company-info">
              ...
              <div>State Name : ${safeText(invoice.company_state)}${invoice.company_state_code ? `, Code : ${safeText(invoice.company_state_code)}` : ''}</div>
        ...
        <div class="right-col">
          ...
          <div class="detail-row">
            <div class="detail-cell w25">Due Date</div>
            <div class="detail-cell w25">${invoice.due_date ? formatDate(invoice.due_date) : ''}</div>
            <div class="detail-cell w50">Mode/Terms of Payment</div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Reference No.</div>
            <div class="detail-cell w25">${safeText(invoice.reference_number) || ''}</div>
    ...
    <div class="footer">
      <div class="footer-left">
        ...
        ${invoice.terms_and_conditions ? `<br><br><strong>Terms & Conditions:</strong><br>${safeText(invoice.terms_and_conditions).replace(/\n/g, '<br>')}` : ''}
        ${invoice.bank_name || invoice.bank_account_number ? `
        <div class="bank-details">
          <div class="bank-details-title">Bank Details:</div>
          ${invoice.bank_name ? `Bank Name: ${safeText(invoice.bank_name)}<br>` : ''}
          ${invoice.bank_account_number ? `A/c No: ${safeText(invoice.bank_account_number)}<br>` : ''}
          ${invoice.bank_ifsc ? `IFSC Code: ${safeText(invoice.bank_ifsc)}<br>` : ''}
          ${invoice.bank_branch ? `Branch: ${safeText(invoice.bank_branch)}` : ''}
        </div>
        ` : ''}
      </div>
