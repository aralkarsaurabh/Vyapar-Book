...
function generateDebitNoteHTML({ debitNote, items }) {
  const isIntraState = debitNote.company_state === debitNote.vendor_state;
  const taxBreakdown = {};
  ...
  // Generate GST rows
  let gstRows = '';
  if (isIntraState && Number(debitNote.cgst_total) > 0) {
    gstRows = `
  ...
  } else if (!isIntraState && Number(debitNote.igst_total) > 0) {
    gstRows = `
        <div class="item-row gst-row">
          <div class="col-sl"></div>
...
<body>
  <div class="invoice-page">
    ...
    <div class="ref-box">
      <strong>Against Bill:</strong> ${safeText(debitNote.original_bill_number)} ${debitNote.original_bill_date ? `dated ${formatDate(debitNote.original_bill_date)}` : ''}
    </div>

    <div class="reason-box">
      <strong>Reason:</strong> ${safeText(debitNote.reason)}${debitNote.reason_notes ? ` - ${safeText(debitNote.reason_notes)}` : ''}
    </div>
      ...
      <div class="main-row">
        <div class="left-col">
          ...
          <div class="buyer-section">
            ...
            <div class="buyer-name">${safeText(debitNote.vendor_name)}</div>
            <div class="buyer-details">
              ${debitNote.vendor_address_line1 ? `${safeText(debitNote.vendor_address_line1)}<br>` : ''}
              ${debitNote.vendor_address_line2 ? `${safeText(debitNote.vendor_address_line2)}<br>` : ''}
              ${debitNote.vendor_city ? `${safeText(debitNote.vendor_city)}<br>` : ''}
              ${debitNote.vendor_gst ? `GSTIN/UIN : ${safeText(debitNote.vendor_gst)}<br>` : ''}
              State Name : ${safeText(debitNote.vendor_state)}${debitNote.vendor_state_code ? `, Code : ${safeText(debitNote.vendor_state_code)}` : ''}
            </div>
          </div>
        </div>
        ...
        <div class="right-col">
          ...
          <div class="detail-row">
            <div class="detail-cell w25">Original Bill No.</div>
            <div class="detail-cell w25">${safeText(debitNote.original_bill_number)}</div>
            <div class="detail-cell w25">Original Bill Date</div>
            <div class="detail-cell w25">${formatDate(debitNote.original_bill_date)}</div>
          </div>
          <div class="detail-row">
    ...
    <div class="footer">
      <div class="footer-left">
        ...
        This debit note is issued against the original bill mentioned above for the purpose of adjusting the purchase amount due to the stated reason.
