import { generateQuotationHTML } from './quotation-html-template';
...
export async function generateQuotationPDF(
  ...
  const quotationResult = await query(
    `SELECT q.*,
     ...
     JOIN company_profiles comp ON q.company_id = comp.company_id
     WHERE q.quotation_id = $1 AND q.user_id = $2`,
    [quotationId, userId]
  );
  ...
  // 3️⃣ Normalize data
  const quotationData = normalizeObject(quotationResult.rows[0]);
