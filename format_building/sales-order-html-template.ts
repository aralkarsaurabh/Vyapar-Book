...
export function generateSalesOrderHTML({ salesOrder, items }: SalesOrderData): string {
  const isIntraState = salesOrder.company_state === salesOrder.customer_state;
...
<head>
  ...
  <title>Sales Order - ${safeText(salesOrder.so_number)}</title>
  <style>
    @page {
      size: A4;
      margin: 10mm;
  ...
  <div class="invoice-page">
      ...
      <div class="main-row">
        <div class="left-col">
          <div class="company-header">
            <div class="logo-box">
              ${salesOrder.company_logo
      ? `<img src="${safeText(salesOrder.company_logo)}" alt="Company Logo" />`
      : `<div class="logo-placeholder"><div class="logo-triangle"></div></div>`
    }
            </div>
            <div class="company-info">
              <div class="company-name">${safeText(salesOrder.company_name)}</div>
              ${salesOrder.company_address_line1 ? `<div>${safeText(salesOrder.company_address_line1)},</div>` : ''}
              ${salesOrder.company_address_line2 ? `<div>${safeText(salesOrder.company_address_line2)}</div>` : ''}
              ${salesOrder.company_city ? `<div>${safeText(salesOrder.company_city)}</div>` : ''}
              <div>GSTIN/UIN: ${safeText(salesOrder.company_gst) || 'N/A'}</div>
              <div>State Name : ${safeText(salesOrder.company_state)}${salesOrder.company_state_code ? `, Code : ${safeText(salesOrder.company_state_code)}` : ''}</div>
              ${salesOrder.company_email ? `<div>E-Mail : ${safeText(salesOrder.company_email)}</div>` : ''}
            </div>
          </div>

          <div class="consignee-section">
            <div class="buyer-label">Consignee (Ship to)</div>
            <div class="buyer-name">${safeText(salesOrder.customer_name)}</div>
            <div class="buyer-details">
              ${salesOrder.customer_address_line1 ? `${safeText(salesOrder.customer_address_line1)}<br>` : ''}
              ${salesOrder.customer_address_line2 ? `${safeText(salesOrder.customer_address_line2)}<br>` : ''}
              ${salesOrder.customer_city ? `${safeText(salesOrder.customer_city)}<br>` : ''}
              ${salesOrder.customer_gst ? `GSTIN/UIN : ${safeText(salesOrder.customer_gst)}<br>` : ''}
              State Name : ${safeText(salesOrder.customer_state)}${salesOrder.customer_state_code ? `, Code : ${safeText(salesOrder.customer_state_code)}` : ''}
            </div>
          </div>
          ...
          <div class="buyer-section">
            ...
            <div class="buyer-details">
              ...
              ${salesOrder.customer_city ? `${safeText(salesOrder.customer_city)}<br>` : ''}
              ${salesOrder.customer_gst ? `GSTIN/UIN : ${safeText(salesOrder.customer_gst)}<br>` : ''}
              State Name : ${safeText(salesOrder.customer_state)}, Code : ${safeText(salesOrder.customer_state_code) || ''}
            </div>
          </div>
        </div>
        ...
        <div class="right-col">
          <div class="detail-row">
            <div class="detail-cell w25">SO Number</div>
            <div class="detail-cell w25">${safeText(salesOrder.so_number)}</div>
            <div class="detail-cell w25">Dated</div>
            <div class="detail-cell w25">${formatDate(salesOrder.so_date)}</div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Expected Delivery</div>
            <div class="detail-cell w25">${salesOrder.expected_delivery_date ? formatDate(salesOrder.expected_delivery_date) : ''}</div>
            <div class="detail-cell w50">Payment Terms</div>
          </div>
          <div class="detail-row">
          ...
          <div class="detail-row">
            <div class="detail-cell w25">Customer PO</div>
            <div class="detail-cell w25"></div>
            <div class="detail-cell w25">Dated</div>
            <div class="detail-cell w25"></div>
          </div>
          <div class="detail-row">
            ...
            <div class="detail-cell w25">${safeText(salesOrder.place_of_supply) || ''}</div>
            <div class="detail-cell w25">Status</div>
            <div class="detail-cell w25">${safeText(salesOrder.status).toUpperCase()}</div>
          </div>
    ...
    <div class="items-section">
      ...
      return `
        ...
        ${isIntraState && Number(salesOrder.cgst_total) > 0 ? `
        <div class="item-row gst-row">
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(salesOrder.cgst_total))}</div>
        </div>
        <div class="item-row gst-row">
          <div class="col-sl"></div>
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(salesOrder.sgst_total))}</div>
        </div>
        ` : ''}

        ${!isIntraState && Number(salesOrder.igst_total) > 0 ? `
        <div class="item-row gst-row">
          <div class="col-sl"></div>
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(salesOrder.igst_total))}</div>
        </div>
        ` : ''}
      </div>
      ...
      <div class="total-row">
        ...
        <div class="col-rate"></div>
        <div class="col-per"></div>
        <div class="col-amt grand-total">₹ ${formatINRForPDF(Number(salesOrder.grand_total))}</div>
      </div>
    </div>
    ...
    <div class="amount-words">
      <div class="amount-words-left">
        ...
        <span class="amount-words-value">${amountToWords(Number(salesOrder.grand_total))}</span>
      </div>
      <div class="amount-words-right">E. & O.E</div>
    </div>

    <!-- Tax Summary Table -->
    ${items.some(i => Number(i.cgst_rate) > 0 || Number(i.igst_rate) > 0) ? `
    <table class="tax-table">
      ...
      <tbody>
        ...
        <tr style="font-weight:bold;">
          <td>Total</td>
          <td class="text-right">${formatINRForPDF(Number(salesOrder.subtotal))}</td>
          ${isIntraState ? `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(salesOrder.cgst_total))}</td>
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(salesOrder.sgst_total))}</td>
          ` : `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(salesOrder.igst_total))}</td>
          `}
          <td class="text-right">${formatINRForPDF(Number(salesOrder.tax_total))}</td>
        </tr>
      </tbody>
    </table>

    <div class="tax-words">
      <strong>Tax Amount (in words) :</strong> ${amountToWords(Number(salesOrder.tax_total))}
    </div>
    ` : ''}
    ...
    <div class="footer">
      <div class="footer-left">
        ...
        This sales order confirms pricing and specifications.
        ${salesOrder.notes ? `<br><br><strong>Notes:</strong><br>${safeText(salesOrder.notes)}` : ''}
      </div>
      <div class="footer-right">
        <div>for <strong>${safeText(salesOrder.company_name)}</strong></div>
        <div class="signature">Authorised Signatory</div>
      </div>
    </div>

    <div class="computer-note">This is a Computer Generated Sales Order</div>
  </div>
</body>
</html>
  `.trim();
