...
export function generateDebitNoteHTML({ debitNote, items }: DebitNoteData): string {
  const isIntraState = debitNote.company_state === debitNote.customer_state;
...
<head>
  ...
  <title>Debit Note - ${safeText(debitNote.debit_note_number)}</title>
  <style>
    @page {
      size: A4;
      margin: 10mm;
...
<body>
  <div class="invoice-page">
    ...
    <!-- Debit Note Specific: Ref Box -->
    <div class="ref-box">
        <strong>Original Invoice:</strong> ${safeText(debitNote.original_invoice_number)} dated ${formatDate(debitNote.original_invoice_date)}
    </div>
    
    <!-- Debit Note Specific: Reason Box -->
    <div class="reason-box">
        <strong>Reason:</strong> ${safeText(debitNote.reason)}
    </div>
      ...
      <div class="main-row">
        ...
        <div class="left-col">
          ...
          <div class="company-header">
            <div class="logo-box">
              ${debitNote.company_logo
      ? `<img src="${safeText(debitNote.company_logo)}" alt="Company Logo" />`
      : `<div class="logo-placeholder"><div class="logo-triangle"></div></div>`
    }
            </div>
            <div class="company-info">
              <div class="company-name">${safeText(debitNote.company_name)}</div>
              ${debitNote.company_address_line1 ? `<div>${safeText(debitNote.company_address_line1)},</div>` : ''}
              ${debitNote.company_address_line2 ? `<div>${safeText(debitNote.company_address_line2)}</div>` : ''}
              ${debitNote.company_city ? `<div>${safeText(debitNote.company_city)}</div>` : ''}
              <div>GSTIN/UIN: ${safeText(debitNote.company_gst) || 'N/A'}</div>
              <div>State Name : ${safeText(debitNote.company_state)}${debitNote.company_state_code ? `, Code : ${safeText(debitNote.company_state_code)}` : ''}</div>
              ${debitNote.company_email ? `<div>E-Mail : ${safeText(debitNote.company_email)}</div>` : ''}
            </div>
          </div>

          <!-- Buyer (Bill to) Section -->
          <div class="buyer-section">
            <div class="buyer-label">Customer (Bill to)</div>
            <div class="buyer-name">${safeText(debitNote.customer_name)}</div>
            <div class="buyer-details">
              ${debitNote.customer_address_line1 ? `${safeText(debitNote.customer_address_line1)}<br>` : ''}
              ${debitNote.customer_address_line2 ? `${safeText(debitNote.customer_address_line2)}<br>` : ''}
              ${debitNote.customer_city ? `${safeText(debitNote.customer_city)}<br>` : ''}
              ${debitNote.customer_gst ? `GSTIN/UIN : ${safeText(debitNote.customer_gst)}<br>` : ''}
              State Name : ${safeText(debitNote.customer_state)}${debitNote.customer_state_code ? `, Code : ${safeText(debitNote.customer_state_code)}` : ''}
            </div>
          </div>
        </div>

        <!-- Right Column: Debit Note Details -->
        <div class="right-col">
          <div class="detail-row">
            <div class="detail-cell w25">Debit Note No.</div>
            <div class="detail-cell w25">${safeText(debitNote.debit_note_number)}</div>
            <div class="detail-cell w25">Dated</div>
            <div class="detail-cell w25">${formatDate(debitNote.debit_note_date)}</div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Original Inv. No.</div>
            <div class="detail-cell w25">${safeText(debitNote.original_invoice_number)}</div>
            <div class="detail-cell w25">Original Inv. Date</div>
            <div class="detail-cell w25">${formatDate(debitNote.original_invoice_date)}</div>
          </div>
          <div class="detail-row">
            <div class="detail-cell w25">Place of Supply</div>
            <div class="detail-cell w25">${safeText(debitNote.place_of_supply)}</div>
             <div class="detail-cell w50"></div>
          </div>
          <div class="detail-row" style="min-height:25px;">
            <div class="detail-cell w100">Reference</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="items-section">
      ...
      return `
        ...
        <!-- Output CGST/SGST rows -->
        ${isIntraState && Number(debitNote.cgst_total) > 0 ? `
        <div class="item-row gst-row">
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(debitNote.cgst_total))}</div>
        </div>
        <div class="item-row gst-row">
          <div class="col-sl"></div>
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(debitNote.sgst_total))}</div>
        </div>
        ` : ''}

        ${!isIntraState && Number(debitNote.igst_total) > 0 ? `
        <div class="item-row gst-row">
          <div class="col-sl"></div>
          ...
          <div class="col-rate"></div>
          <div class="col-per"></div>
          <div class="col-amt">${formatINRForPDF(Number(debitNote.igst_total))}</div>
        </div>
        ` : ''}
      </div>
      ...
      <div class="total-row">
        ...
        <div class="col-rate"></div>
        <div class="col-per"></div>
        <div class="col-amt grand-total">₹ ${formatINRForPDF(Number(debitNote.grand_total))}</div>
      </div>
    </div>
    ...
    <div class="amount-words">
      <div class="amount-words-left">
        Amount (in words)<br>
        <span class="amount-words-value">${amountToWords(Number(debitNote.grand_total))}</span>
      </div>
    ...
    <table class="tax-table">
      ...
      <tbody>
        ...
        <tr style="font-weight:bold;">
          <td>Total</td>
          <td class="text-right">${formatINRForPDF(Number(debitNote.subtotal))}</td>
          ${isIntraState ? `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(debitNote.cgst_total))}</td>
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(debitNote.sgst_total))}</td>
          ` : `
          <td></td>
          <td class="text-right">${formatINRForPDF(Number(debitNote.igst_total))}</td>
          `}
          <td class="text-right">${formatINRForPDF(Number(debitNote.tax_total))}</td>
        </tr>
      </tbody>
    </table>
    ...
    <!-- Tax Amount in Words -->
    <div class="tax-words">
      <strong>Tax Amount (in words) :</strong> ${amountToWords(Number(debitNote.tax_total))}
    </div>
    ...
    <div class="footer">
      <div class="footer-left">
        ...
        This debit note is issued against the original invoice mentioned above.
        ${debitNote.notes ? `<br><br><strong>Notes:</strong><br>${safeText(debitNote.notes)}` : ''}
      </div>
      <div class="footer-right">
        <div>for <strong>${safeText(debitNote.company_name)}</strong></div>
        <div class="signature">Authorised Signatory</div>
      </div>
    </div>

    <!-- Computer Generated Note -->
    <div class="computer-note">This is a Computer Generated Debit Note</div>
  </div>
</body>
</html>
  `.trim();
