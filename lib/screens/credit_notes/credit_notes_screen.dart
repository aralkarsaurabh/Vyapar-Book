...
class _CreditNotesScreenState extends State<CreditNotesScreen> {
  ...
  Future<void> _deleteCreditNote(CreditNote creditNote) async {
    // Don't allow deleting sent credit notes
    if (creditNote.status == CreditNoteStatus.sent) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cannot delete a credit note that has been sent'),
    ...
    if (confirm == true) {
      final success = await _creditNoteService.deleteCreditNote(creditNote.id!);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(success ? 'Credit note deleted successfully' : 'Failed to delete credit note'),
                  ...
                  children: [
                    ...
                    Text(
                      _viewMode == ViewMode.created
                          ...
                          : 'Credit notes received from vendors',
                ...
                Row(
                  children: [
                    ...
                    if (_viewMode == ViewMode.created)
                      ElevatedButton.icon(
                        onPressed: () => context.go('/dashboard/credit-notes/create'),
            ...
            Expanded(
              child: Container(
                ...
                child: _viewMode == ViewMode.created
                    ? _buildCreatedCreditNotesStream()
                    : _buildReceivedCreditNotesStream(),
              ),
            ),
          ],
        ),
  ...
  Widget _buildCreatedCreditNotesStream() {
    return StreamBuilder<List<CreditNote>>(
      stream: _creditNoteService.getCreditNotes(),
      builder: (context, snapshot) {
        ...
        final allCreditNotes = snapshot.data ?? [];

        if (allCreditNotes.isEmpty) {
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                ...
                const SizedBox(height: 8),
                const Text(
                  'Create a credit note when you need to issue a refund or adjustment',
                  style: TextStyle(
                    fontSize: 14,
                    color: AppColors.textSecondary,
                  ),
        ...
        // Pagination logic
        final totalItems = allCreditNotes.length;
        ...
        final creditNotes = allCreditNotes.sublist(startIndex, endIndex);
              ...
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    ...
                    // Table Rows
                    ...creditNotes.map((creditNote) => _buildCreatedTableRow(creditNote)),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildReceivedCreditNotesStream() {
    return StreamBuilder<List<SharedDocument>>(
      stream: _sharedDocumentService.getReceivedCreditNotes(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    needsIndex
                        ? 'Firestore Index Required'
                        : 'Error loading received credit notes',
                    style: const TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.w600,
                      color: AppColors.textPrimary,
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text(
                  'Credit notes sent to your Vyapar ID will appear here',
                  style: TextStyle(
                    fontSize: 14,
                    color: AppColors.textSecondary,
                  ),
  ...
  Widget _buildReasonBadge(String? reason) {
    ...
    switch (reason) {
      case CreditNoteReason.goodsReturned:
        shortReason = 'Return';
        bgColor = AppColors.info.withOpacity(0.1);
        textColor = AppColors.info;
        break;
      case CreditNoteReason.discountGiven:
        shortReason = 'Discount';
        bgColor = AppColors.success.withOpacity(0.1);
        textColor = AppColors.success;
        break;
      case CreditNoteReason.overchargeCorrection:
        shortReason = 'Correction';
        bgColor = AppColors.warning.withOpacity(0.1);
        textColor = AppColors.warning;
        break;
  ...
  Widget _buildCreatedTableRow(CreditNote creditNote) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 16),
      decoration: BoxDecoration(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 2,
            child: Text(
              creditNote.againstInvoiceNumber ?? '-',
              style: const TextStyle(
                color: AppColors.textPrimary,
                fontSize: 14,
              ),
          ...
          Expanded(
            flex: 1,
            child: Text(
              creditNote.creditNoteDate != null
                  ? _dateFormat.format(creditNote.creditNoteDate!)
                  : '-',
              style: const TextStyle(
                color: AppColors.textPrimary,
                fontSize: 14,
          ...
          Expanded(
            flex: 1,
            child: _buildReasonBadge(creditNote.reason),
          ),
          Expanded(
            flex: 1,
            child: Text(
              _currencyFormat.format(creditNote.grandTotal),
              style: const TextStyle(
                fontWeight: FontWeight.w600,
                color: AppColors.textPrimary,
                fontSize: 14,
          ...
          SizedBox(
            ...
            child: PopupMenuButton<String>(
              ...
              onSelected: (value) {
                switch (value) {
                  case 'view':
                    context.go('/dashboard/credit-notes/view/${creditNote.id}');
                    break;
                  case 'delete':
                    _deleteCreditNote(creditNote);
                    break;
                }
              },
              itemBuilder: (context) => [
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 2,
            child: Text(
              doc.documentSnapshot?['againstInvoiceNumber'] ?? '-',
              style: const TextStyle(
                color: AppColors.textPrimary,
                fontSize: 14,
              ),
          ...
          SizedBox(
            ...
            child: IconButton(
              ...
              tooltip: 'View',
              onPressed: () {
                context.go('/dashboard/credit-notes/view-received/${doc.id}');
              },
            ),
          ),
        ],
