...
class ViewDebitNoteScreen extends StatefulWidget {
  final String debitNoteId;

  const ViewDebitNoteScreen({super.key, required this.debitNoteId});

  @override
  State<ViewDebitNoteScreen> createState() => _ViewDebitNoteScreenState();
}

class _ViewDebitNoteScreenState extends State<ViewDebitNoteScreen> {
  ...
  Future<void> _loadDebitNote() async {
    setState(() => _isLoading = true);

    final debitNote = await _debitNoteService.getDebitNoteById(widget.debitNoteId);
  ...
  Future<void> _sendToVendor() async {
    if (_debitNote == null || _debitNote!.vendorVyaparId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Vendor does not have a Vyapar ID'),
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Send Debit Note'),
        content: Text('Send this debit note to ${_debitNote!.vendorName} (${_debitNote!.vendorVyaparId})?'),
        actions: [
          TextButton(
    ...
    try {
      ...
      final vendorInfo = await _sharedDocumentService.searchUserByVyaparId(_debitNote!.vendorVyaparId!);

      if (vendorInfo == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Vendor not found on VyaparBook'),
              backgroundColor: AppColors.error,
            ),
          );
        }
      ...
      final success = await _sharedDocumentService.shareDocument(
        ...
        documentId: _debitNote!.id!,
        documentData: _debitNote!.toMap(),
        receiverVyaparId: _debitNote!.vendorVyaparId!,
        receiverUserId: vendorInfo['userId'],
      );

      if (success) {
        // Mark as sent
        await _debitNoteService.markAsSent(_debitNote!.id!);
        await _loadDebitNote();

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Debit note sent successfully'),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Failed to send debit note'),
              backgroundColor: AppColors.error,
            ),
          );
        }
      ...
      debugPrint('Error sending debit note: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              const Text('Debit note not found', style: TextStyle(fontSize: 18, color: AppColors.textPrimary)),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _buildStatusBadge(_debitNote!.status),
                ...
                // Action buttons
                if (_debitNote!.status != DebitNoteStatus.sent && _debitNote!.vendorVyaparId != null) ...[
                  ElevatedButton.icon(
                    onPressed: _isSending ? null : _sendToVendor,
                    icon: _isSending
                        ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                        : const Icon(Icons.send),
                    label: Text(_isSending ? 'Sending...' : 'Send to Vendor'),
  ...
  Widget _buildVendorInfoSection() {
    return Column(
      children: [
        _buildDetailRow('Vendor', _debitNote!.vendorName ?? '-'),
        _buildDetailRow('Vyapar ID', _debitNote!.vendorVyaparId ?? '-'),
        _buildDetailRow('Place of Supply', _debitNote!.placeOfSupply ?? '-'),
      ],
    );
  }
  ...
  Widget _buildDebitNoteDetailsSection() {
    return Column(
      children: [
        _buildDetailRow('Against Bill', _debitNote!.againstBillNumber ?? '-'),
