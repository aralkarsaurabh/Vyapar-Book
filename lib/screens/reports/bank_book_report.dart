...
class BankBookReport extends StatefulWidget {
  const BankBookReport({super.key});

  @override
  State<BankBookReport> createState() => _BankBookReportState();
}
...
class _BankBookReportState extends State<BankBookReport> {
  ...
  Future<void> _loadBankAccounts() async {
    final banks = await _reportService.getBankAccountList();
    if (mounted) {
      setState(() {
        _bankAccounts = banks;
        _isLoadingBanks = false;
        if (banks.isNotEmpty) {
          _selectedBankId = banks.first['id'] as String;
          _loadReport();
        }
      });
    }
              ...
              children: [
                ...
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Bank Book', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                      SizedBox(height: 4),
                      Text('All bank transactions per bank account', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
                    ],
                  ),
                ),
              ],
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              const Text('Bank: ', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
              ...
              Expanded(
                child: DropdownButtonFormField<String>(
                  ...
                  hint: const Text('Select bank account', style: TextStyle(fontSize: 13)),
                  items: _bankAccounts.map((bank) {
                    return DropdownMenuItem<String>(
                      value: bank['id'] as String,
                      child: Text(bank['name'] as String, style: const TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis),
                    );
                  }).toList(),
                  onChanged: (value) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text('No bank accounts found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
          const SizedBox(height: 8),
          const Text('Add bank details in your company profile to see bank transactions', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
        ],
      ),
    );
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.account_balance, size: 64, color: AppColors.textSecondary.withOpacity(0.5)),
          const SizedBox(height: 16),
          const Text('No bank transactions found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
          const SizedBox(height: 8),
      ...
      child: Column(
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.05), border: Border(bottom: BorderSide(color: AppColors.border))),
            child: Row(
              children: [
                Text('Bank: ${_ledgerData!.accountName}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.primary)),
              ],
            ),
          ),
          Container(
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                Expanded(flex: 2, child: Text('Deposit', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary), textAlign: TextAlign.right)),
                Expanded(flex: 2, child: Text('Withdrawal', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary), textAlign: TextAlign.right)),
