...
class PayablesAgingReport extends StatefulWidget {
  const PayablesAgingReport({super.key});

  @override
  State<PayablesAgingReport> createState() => _PayablesAgingReportState();
}

class _PayablesAgingReportState extends State<PayablesAgingReport> {
  final _reportService = ReportService();
  final _currencyFormat = NumberFormat.currency(locale: 'en_IN', symbol: '\u20B9', decimalDigits: 2);
  final _dateFormat = DateFormat('dd MMM yyyy');

  List<PayablesAgingItem> _items = [];
  ...
  Future<void> _loadReport() async {
    ...
    final items = await _reportService.getPayablesAging();
    if (mounted) {
      setState(() {
        _items = items;
        _isLoading = false;
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final savedPath = await ReportPdfService.saveReportPdf(pdfBytes, 'payables_aging.pdf');
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Payables Aging', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                      const SizedBox(height: 4),
                      Text('As on ${_dateFormat.format(DateTime.now())} - overdue analysis for vendor payments', style: const TextStyle(fontSize: 14, color: AppColors.textSecondary)),
                    ],
                  ),
                ),
                OutlinedButton.icon(
  ...
  Widget _buildSummaryCards() {
    ...
    return Row(
      children: [
        _buildSummaryCard('Total Outstanding', totalOutstanding, AppColors.warning),
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        ...
                        child: Row(
                          children: [
                            Flexible(child: Text(item.vendorName, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.textPrimary), overflow: TextOverflow.ellipsis)),
                            if (hasOverdue60) ...[
