...
class CustomerWiseSalesReport extends StatefulWidget {
  const CustomerWiseSalesReport({super.key});

  @override
  State<CustomerWiseSalesReport> createState() => _CustomerWiseSalesReportState();
}

class _CustomerWiseSalesReportState extends State<CustomerWiseSalesReport> {
  final _reportService = ReportService();
  final _currencyFormat = NumberFormat.currency(locale: 'en_IN', symbol: '₹', decimalDigits: 2);
  final _dateFormat = DateFormat('dd MMM yyyy');

  List<CustomerWiseSalesItem> _items = [];
  ...
  Future<void> _loadReport() async {
    ...
    final items = await _reportService.getCustomerWiseSales(
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final pdfBytes = await ReportPdfService.generateReportPdf(
        reportType: 'customer_wise_sales',
        items: items,
        startDate: _startDate,
        endDate: _endDate,
      );

      final savedPath = await ReportPdfService.saveReportPdf(pdfBytes, 'customer_wise_sales.pdf');
              ...
              children: [
                ...
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Customer-wise Sales', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                      SizedBox(height: 4),
                      Text('Sales breakdown per customer', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
                    ],
                  ),
                ),
                OutlinedButton.icon(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text('No sales data found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
  ...
  Widget _buildTable() {
    int totalCount = 0;
    double totalSales = 0, totalGst = 0, totalAmount = 0;
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                Expanded(flex: 2, child: Text('Sales Amount', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary), textAlign: TextAlign.right)),
                ...
                return Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(flex: 2, child: Text(_currencyFormat.format(item.salesAmount), style: const TextStyle(fontSize: 13), textAlign: TextAlign.right)),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(flex: 2, child: Text(_currencyFormat.format(totalSales), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold), textAlign: TextAlign.right)),
