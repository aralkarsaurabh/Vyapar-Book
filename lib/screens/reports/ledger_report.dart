...
class _LedgerReportState extends State<LedgerReport> {
  ...
  Future<void> _loadAccounts() async {
    final accounts = await _reportService.getAccountList();
    if (mounted) {
      setState(() {
        _accounts = accounts;
              ...
              children: [
                ...
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Ledger Report', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                      SizedBox(height: 4),
                      Text('All transactions for a specific account', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
                    ],
                  ),
                ),
              ],
            ...
            children: [
              ...
              Expanded(
                child: DropdownButtonFormField<String>(
                  ...
                  hint: const Text('Select an account', style: TextStyle(fontSize: 13)),
  ...
  Widget _buildSelectAccountState() {
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text('Select an account', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
          const SizedBox(height: 8),
          const Text('Choose an account from the dropdown to view its ledger', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
        ],
      ),
    );
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text('No entries for this account in the selected date range', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Text('Ledger: ${_ledgerData!.accountName}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.primary)),
                const Spacer(),
                Text('Type: ${_ledgerData!.accountType.toUpperCase()}', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
              ],
            ),
          ),
          // Opening balance row
                      ...
                      Expanded(flex: 4, child: Column(
                        ...
                        children: [
                          ...
                          if (item.reference.isNotEmpty)
                            Text(item.reference, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                        ],
                      )),
                      Expanded(flex: 2, child: Text(item.debit > 0 ? _currencyFormat.format(item.debit) : '', style: const TextStyle(fontSize: 13, color: AppColors.textPrimary), textAlign: TextAlign.right)),
                      Expanded(flex: 2, child: Text(item.credit > 0 ? _currencyFormat.format(item.credit) : '', style: const TextStyle(fontSize: 13, color: AppColors.textPrimary), textAlign: TextAlign.right)),
